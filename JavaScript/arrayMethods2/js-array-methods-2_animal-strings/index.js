...
const catStartingFromIndexFiveExists = animalStrings.findIndex((animal, index) => index >= 5 && animal === 'cat') !== -1;

// Hint: Besides the array method, check out the string method `startsWith()`.
const firstAnimalStartingWithLetterP = animalStrings.find((animalString) => animalString.startsWith("p"));

const indexOfGiraffe = animalStrings.findIndex((animalString) =>animalString.startsWith('g'));
...
const anyAnimalEndsWithLetterZ = animalStrings.some((animals) =>animals.endsWith('z'));

const everyAnimalHasMoreThanTwoLetters = animalStrings.every((twoLetters) => twoLetters.length > 2);
...
const sumOfAllAnimalCharacters = animalStrings.map(animalC => animalC.length).reduce((sum, length) => sum + length);
